...
const AddFlashSale = ({ close, fetchData }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        for (const item of selectedProducts) {
            ...
            if (item.flashSaleStock > product.stock) {
                toast.error(`Số lượng Flash Sale của ${product.name} không thể lớn hơn tồn kho`);
                return;
            }
        ...
        try {
            const response = await Axios({
                ...SummaryApi.create_flash_sale,
                data: dataToSend
            });

            if (response.data.success) {
                toast.success(response.data.message || 'Tạo Flash Sale thành công');
                fetchData();
                close();
            }
        } catch (error) {
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                                    ...
                                    <table className="min-w-full divide-y divide-gray-200">
                                        <thead className="bg-gray-50">
                                            <tr>
                                                ...
                                                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Giới hạn / người</th>
                                            </tr>
                                        </thead>
                        ...
                        <div className="flex justify-end space-x-3 pt-4">
                            ...
                            <button
                                ...
                                disabled={loading}
                            >
                                {loading ? 'Đang xử lý...' : 'Tạo Flash Sale'}
                            </button>
                        </div>
                    </form>
                </div>
